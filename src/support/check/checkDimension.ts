 ...
 */
export default async (selector: Selector, falseCase: boolean, expectedSize: string, dimension: 'broad' | 'tall') => {
	/**
	...
	if (falseCase) {
		...
		expect(originalSize).toBe(
			intExpectedSize,
			// @ts-expect-error
			`Element "${selector}" should have a ${label} of ` + `${intExpectedSize}px, but is ${originalSize}px`
		);
	}
};
