 ...
 */
export default async (selector: Selector, falseCase: boolean, expectedPosition: string, axis: 'x' | 'y') => {
	/**
	...
	if (falseCase) {
		expect(location).not.toEqual(
			intExpectedPosition,
			// @ts-expect-error
			`Element "${selector}" should not be positioned at ` + `${intExpectedPosition}px on the ${axis} axis`
		);
	} else {
